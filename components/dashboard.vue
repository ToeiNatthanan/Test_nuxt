    ...
    <html lang="en">
      ...
      <body class="g-sidenav-show bg-gray-100">
        ...
        >
          ...
          <div class="sidenav-footer mx-3 mt-n4 mb-50">
            <div
...
export default {
  ...
  data() {
    return {
      bitcoinPrice: "$0.0",
      bitcoinChange: "0%",
      bitcoinChangeClass: "text-success",
      ethereumPrice: "$0.0",
      ethereumChange: "0%",
      ethereumChangeClass: "text-success",
      solanaPrice: "$0.0",
      solanaChange: "0%",
      solanaChangeClass: "text-success",
      dogecoinPrice: "$0.0",
      dogecoinChange: "0%",
      dogecoinChangeClass: "text-success",
      cryptos: [],
    };
  },
  async mounted() {
  ...
  methods: {
    async fetchCryptoData(assetId, priceKey, changeKey, changeClassKey) {
      try {
        const response = await fetch(
          `https://komgrip.co.th/coincap/assets?search=${assetId}`
        );
        const data = await response.json();

        const assetData = data.data.find((asset) => asset.id === assetId);

        if (assetData) {
          this[priceKey] = `$${parseFloat(assetData.priceUsd).toFixed(2)}`;
          const change = parseFloat(assetData.changePercent24Hr).toFixed(2);
          this[changeKey] = `${change}%`;
          this[changeClassKey] = change >= 0 ? "text-success" : "text-danger";
        }
      } catch (error) {
    ...
    async fetchAllCryptoData() {
      try {
        const response = await fetch(
          "https://komgrip.co.th/coincap/assets?limit=5"
        );
        const data = await response.json();
    ...
    formatSupply(value) {
      return value ? parseFloat(value).toLocaleString() : "No Limit";
    },
  },
};
</script>
